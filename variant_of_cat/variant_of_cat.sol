...
contract baseRule{
    ...
    modifier isPlayer(address requester) {
        require(catToOwner[ownerToCat[requester]]==requester,"表哥，领猫吗?");
        _;
    }
    function _triggerCoolCountDown(uint catID) internal{
        cats[catID].coolcountdown = block.timestamp + 20 seconds;
    }
    function _fight(uint catID,Cat catTarget) internal returns(bool) {
...
contract variant_of_cat is catStore {
    ...
    function deleteMyCat() external isPlayer(msg.sender) {
